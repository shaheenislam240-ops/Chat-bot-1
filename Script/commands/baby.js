...
module.exports.run = async function ({ api, event, args, Users }) {
  try {
    ...
    if (args[0] === 'edit') {
      ...
      if (parts.length < 2) return api.sendMessage('❌ | Invalid format!\nUse: edit [YourMessage] - [NewReply]', event.threadID, event.messageID);
      const original = parts[0].replace("edit ", "").trim();
      const replacement = parts[1].trim();
      if (!original || !replacement) return api.sendMessage('❌ | Please provide both the original message and the new reply.\nFormat: edit [YourMessage] - [NewReply]', event.threadID, event.messageID);
      const res = await axios.get(`${link}?edit=${encodeURIComponent(original)}&replace=${encodeURIComponent(replacement)}`);
      const msg = res.data?.message || "❌ API didn't return any message.";
      return api.sendMessage(`✅ Changed:\n${msg}`, event.threadID, event.messageID);
    }
...
module.exports.handleEvent = async function ({ api, event }) {
  try {
    ...
    const triggerWords = [
      "baby", "bby", "/bot", "bot", "sweetie", "jan", "bbz", "sona",
      "xan", "maria", "babu", "koliza", "baby koi tumi", "maria koi tumi"
    ];
